...
pages.each do |title, body|
  textile = File.join(OUT_PATH, title.gsub(/\s/, '')) + '.textile'
  rst = File.join(OUT_PATH, title.gsub(/\s/, '')) + '.rst'
  File.open(textile, 'w') do |file|
    body.gsub!(/\r/, '')
    body.gsub!(/\{\{\{([^\n]+?)\}\}\}/, '@\1@')
  ...
  puts title
  %x(sudo /root/.cabal/bin/pandoc -f textile -t rst -o #{rst} #{textile})
end
